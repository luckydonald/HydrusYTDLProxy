...
def GetPayloadTypeString(payload_obj):
    if isinstance(payload_obj, bytes):
        ...
        type_string = ', '.join(
            (HydrusNumbers.ToHumanInt(count) + ' ' + s for (s, count) in list(type_string_counts.items())))
        ...
        if len(numpy_image.shape) > 2:
            ...
            if depth != 1:
                numpy_image = numpy_image[:, :,
                              0].copy()  # let's fetch one channel. if the png is a perfect RGB conversion of the original (or, let's say, a Firefox bmp export), this actually works
